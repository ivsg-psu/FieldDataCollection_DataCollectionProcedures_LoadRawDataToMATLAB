function [rawData,varargout] = fcn_LoadRawDataToMATLAB_loadRawDataFromFile(flag_do_debug,varargin)
...
%
% REVISION HISTORY:
% As: fcn_DataClean_loadRawDataFromFile
% 
% 2019_10_03 by Sean Brennan, sbrennan@psu.edu
% - Dr. Brennan revised from Liming Gao's prior version to
%   % comparmentalize this data loading to a structure
% 
% 2019_10_17 by Sean Brennan, sbrennan@psu.edu
% - Removed bad indices from navMode. This was causing problems
...
% - Added debug mode to make the function more verbose.
% 
% 2019_11_22 by Sean Brennan, sbrennan@psu.edu
% - Added centiSeconds field. Needed this to process time jumps
% 
% 2019_11_24 by Sean Brennan, sbrennan@psu.edu
% - Removed NaN fields, as they were causing problems with
%   % standard deviations.
% 
% 2019_11_25 by Sean Brennan, sbrennan@psu.edu
% - Added data consistency checks, as a final step. Fixed bug
%   % in GPS_Novatel data entry.
% 
% 2019_11_26 by Sean Brennan, sbrennan@psu.edu
% - Updated the GPS_Novatel variance for xEast and yNorth
...
% 
% 2020_11_10 by Sean Brennan, sbrennan@psu.edu
% - Renamed function to prep it for DataClean class
% 
% 2020_11_15 by Sean Brennan, sbrennan@psu.edu
% - update so that it can receive varying argument
% 
% 2020_12_07 by Sean Brennan, sbrennan@psu.edu
% - functionalized each sensor data pre-processing
% 
% 2025_09_20 by Sean Brennan, sbrennan@psu.edu
% * In fcn_LoadRawDataToMATLAB_loadRawDataFromFile
% - Renamed function to fcn_LoadRawDataToMATLAB_loadRawDataFromFile
% 
...
if isstring(varargin{1}) || ischar(varargin{1}) % input is a file, filename and variable name
    ...
    varargout{1} = unique(data_struct.Hemisphere_DGPS.GPSWeek); % gps week of time  - Change it to Bin 1 for testing Xinyu's data file
...
try
    Hemisphere = fcn_LoadRawDataToMATLAB_loadRawDataFromFile_Hemisphere(data_struct.Hemisphere_DGPS,data_source,flag_do_debug);
...
try
    GPS_Novatel = fcn_LoadRawDataToMATLAB_loadRawDataFromFile_Novatel_GPS(data_struct.GPS_Novatel,Hemisphere,data_source,flag_do_debug);
...
try
    GPS_Garmin = fcn_LoadRawDataToMATLAB_loadRawDataFromFile_Garmin_GPS(data_struct.Garmin_GPS,data_source,flag_do_debug);
...
try
    IMU_Novatel = fcn_LoadRawDataToMATLAB_loadRawDataFromFile_IMU_Novatel(data_struct.Novatel_IMU,data_source,flag_do_debug);
...
try
    IMU_ADIS = fcn_LoadRawDataToMATLAB_loadRawDataFromFile_IMU_ADIS(data_struct.adis_IMU_data,data_source,flag_do_debug);
...
try
    Input_Steering = fcn_LoadRawDataToMATLAB_loadRawDataFromFile_Input_Steering(data_struct.Steering_angle,data_source,flag_do_debug);
...
try
    Encoder_RearWheels = fcn_LoadRawDataToMATLAB_loadRawDataFromFile_Encoder_RearWheels(data_struct.Raw_encoder,GPS_Novatel, data_source,flag_do_debug);
...
try
    Lidar = fcn_LoadRawDataToMATLAB_loadRawDataFromFile_Lidar(data_struct.Lidar,data_source,flag_do_debug);
    rawData.Lidar = Lidar;
catch
...
fcn_LoadRawDataToMATLAB_checkConsistency(rawData,flag_do_debug);
...
%
% ========================================================================
function fcn_LoadRawDataToMATLAB_checkConsistency(rawData,flag_do_debug)
